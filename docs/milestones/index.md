...
| M3 | [Prompt Engineering & Output Format](M3-PROMPT-ENGINE.md) | **Done** | M2 | v5: LLM outputs JSON classification with `relevant_docs` for document selection. Go composes XML via compose.go with rules-first/intent-last ordering for attention optimization. Scoped rules with source attribution. Intent preserves all specifics from original prompt. Dynamic footer, type-gated guardrails, passthrough detection, versioned prompts |
| M4 | [Server & Dashboard](M4-SERVER-DASHBOARD.md) | **Done** | M1, M2 | Chi server, React SPA (wouter routing), SQLite data layer, SSE hub, streaming pipeline, /api/info endpoint, 4-panel refinement detail, `input_prompt` + `llm_output` DB columns. History page with Timeline/Tool Use/Verifications/Context tabs, real-time SSE updates, session selector with active-first sort. Context page with bootstrap status + document browser + selection history. Global Live indicator in nav |
| M5 | [Rules Engine & Context Gathering](M5-RULES-ENGINE.md) | **~75% Done** | M2 | Rules loader walks to git root, LLM selects rules by index + selects documents by index. ParseRules recognizes `## Anti-Patterns` header. Scoped rules from deep-context documents with source attribution. "After Every Change" = process guardrails, git stripped to branch+commits, LLM generates recent_activity summary, session clips from DB. Needs file mention detection |
...
| M9 | [Testing & Calibration](M9-TESTING.md) | **~55% Done** | M3, M4 | 280 tests across 14 packages. Integration tests use real CLAUDE.md + git. Bootstrap E2E tests cover discover→classify→select→compose. Needs eval framework, >80% coverage |
...
| M11 | [Project Bootstrap & Deep Context](M11-PROJECT-BOOTSTRAP.md) | **Done** | M1, M2, M5 | SessionStart bootstrap (8ms): discovers CLAUDE.md files, generates deep-context docs in .restruct/links/, builds project map (index.json), returns as additionalContext. Deferred LLM classification enriches keywords/summaries after first refine. Lazy re-index on stale files. FileChanged/InstructionsLoaded hooks. Retrieval-augmented refinement: LLM selects relevant docs via `relevant_docs`, compose uses scoped rules with source attribution, rules-first/intent-last ordering. Server endpoints + SSE for bootstrap events. Web: Context page, History Context tab, Sessions bootstrap card, RefinementDetail context selections. 40+ tests |
